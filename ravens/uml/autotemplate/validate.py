...
def _default_hand_template_path() -> Path:
    return Path(__file__).resolve().parents[2] / "lib" / "template.json"


def _default_auto_template_path() -> Path:
...
class AutoTemplateValidator:
    ...
    Usage (interactive):
        v = AutoTemplateValidator(hand_path="ravens/lib/template.json",
    ...
    def from_builder(
        cls,
        builder: Optional["AutoTemplateBuilder"] = None,
        *,
        hand_path: Union[str, Path, None] = None,
        auto_path: Union[str, Path, None] = None,
        hand: Optional[Dict[str, Any]] = None,
        auto: Optional[Dict[str, Any]] = None,
        included_class_names: Optional[Iterable[str]] = None,
        build_if_needed: bool = True,
    ) -> "AutoTemplateValidator":
        """Create a validator directly from an AutoTemplateBuilder.

        This is the most convenient dev workflow for the integrated repo:
        build raw template, grab filtered UML class names from the builder's
        graph, and validate against the hand template in one step.
        """
        from .builder import AutoTemplateBuilder

        if builder is None:
            builder = AutoTemplateBuilder()

        if auto is None:
            if isinstance(getattr(builder, "raw_template", None), dict):
                auto = builder.raw_template
            elif build_if_needed:
                auto = builder.build()

        if included_class_names is None and getattr(builder, "graphs", None) is not None:
            try:
                included_class_names = {
                    str(d.get("Name"))
                    for _, d in builder.graphs.H.nodes(data=True)
                    if isinstance(d, dict) and d.get("Name")
                }
            except Exception:
                included_class_names = None

        return cls(
            hand_path=_default_hand_template_path() if hand_path is None else hand_path,
            auto_path=_default_auto_template_path() if auto_path is None else auto_path,
            hand=hand,
            auto=auto,
...
DevTemplateValidator = AutoTemplateValidator


def validate_against_hand(
    builder: Optional["AutoTemplateBuilder"] = None,
    *,
    hand_path: Union[str, Path, None] = None,
    auto_path: Union[str, Path, None] = None,
    ...
    """Convenience wrapper for the common integrated workflow.

    Returns the validator findings DataFrame directly.
    """
    validator = AutoTemplateValidator.from_builder(
        builder=builder,
        hand_path=hand_path,
        auto_path=auto_path,
        auto=auto,
