...
def add_cim_attributes_to_properties(properties: dict, k: str, v: dict, core_data: CoreData, GG: nx.DiGraph, AT: nx.DiGraph) -> dict:
    try:
        ...
        object_id = (
            core_data.objects[(core_data.objects["Name"] == object_name) & (core_data.objects["Object_Type"] == "Class")].iloc[0]._name
        )
    except:
        raise KeyError(f"Object {object_name} Object_ID not found in CIM UML")
    attribute_ids = {
        a for n in [object_id] + list(nx.ancestors(GG, object_id)) + list(nx.descendants(GG, object_id)) for a in list(AT.predecessors(n))
    }
    for attr_id in attribute_ids:
        attribute = core_data.attributes.loc[attr_id]
        parent = core_data.objects.loc[attribute.Object_ID]
        try:
